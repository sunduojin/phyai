...
every probe and the fallback TorchKernel picks up the work.
"""
...
class FlashInferKernel:
    ...
    ``prefer_for`` is attached at ``register()`` time, not here —
    :class:`phyai.layers.linear.registry.LinearKernelRegistry` owns that.
    """

    name = "flashinfer"

    def supports_capture(self) -> bool:
        # flashinfer kernels JIT-compile on first call and may host-sync,
        # which is unsafe under cudagraph capture.
        return False
