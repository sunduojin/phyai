...
with the eager reference path. Parametrized over both AR and
Diffusion stacks since their flashinfer paged backends are
...
def test_flashinfer_paged_matches_eager_single_sample(flavor: str):
    """flashinfer paged output ~= eager paged output for the same Q/K/V."""
    pool, layer, (H, H_kv, D) = _make_pool_and_layer(flavor)
    N = 4
    torch.manual_seed(0)
    ...
    # Eager reference on a fresh CPU pool.
    pool_cpu = KVCachePool(
        num_layers=1,
        num_slots=16,
        num_kv_heads=H_kv,
        head_dim=D,
    ...
    eager = _factory_for(flavor, "eager")(None)
    cpu_meta = _meta_cls(flavor)(
        mode=AttnMode.PREFILL,
        layout=AttnLayout.RAGGED_3D,
        batch_size=1,
        num_query_tokens=N,
    ...
    eager_plan = eager.init_forward_metadata(cpu_meta)
    ctx_e = _ctx_cls(flavor)(
        backend=eager,
        plan=eager_plan,
        mode=AttnMode.PREFILL,
        layout=AttnLayout.RAGGED_3D,
        kv_pool=pool_cpu,
        write_indices=cpu_meta.write_indices,
    )
    out_e = layer_cpu(q.cpu().float(), k.cpu().float(), v.cpu().float(), ctx_e)
